...
type BtpFile = {
  ...
  ledger?: { transactions?: BtpTx[]; accounts?: BtpFile["budget"] extends infer B ? B extends { accounts?: infer A } ? A : never : never };
};
...
export function convertBtp(data: BtpFile): AppSnapshot {
  ...
  const transactions = txs.map((tx, i) => ({
    ...
    amount: Math.abs(tx.amountOriginal || tx.amountHKD),
    currency: (tx.currency || "HKD") as Transaction["currency"],
